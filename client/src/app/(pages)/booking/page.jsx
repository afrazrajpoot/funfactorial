...
import { bookingFormData } from "@/app/data";

// Dynamically import client-side components with no SSR (to avoid SSR issues)
const BasicDatePicker = dynamic(() => import("@/components/BasicDatePicker"), { ssr: false });
const BasicTimePicker = dynamic(() => import("@/components/BasicTimePicker"), { ssr: false });
const TextField = dynamic(() => import('@mui/material/TextField'), { ssr: false });

const BookingForm = () => {
  const [createBooking, { isLoading, error, isSuccess }] = useCreateBookingMutation();
  const {
    handleSubmit,
    control,
    formState: { errors },
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {/* Form Fields */}
        {bookingFormData && bookingFormData.map((field, index) => (
