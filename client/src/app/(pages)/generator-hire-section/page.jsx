...
// Assuming `products` is fetched or passed down as props
import { products } from "@/app/data";

const GeneratorHierSection = () => {
  const generatorList = [
    {
      info: "PLEASE NOTE: When ordering generators from Fun Factor Leeds, make sure you are fully aware of the power requirements of each item and bear this in mind when selecting a location for your equipment.",
    },
    {
  ...
  const renderProducts = () => {
    if (Array.isArray(products) && products.length > 0) {
      return products.map((elem, ind) => (
        <Card key={ind} {...elem} w="16.5" ml={"3.5vw"} />
      ));
    }
    return <p>No products available</p>;  // Fallback message if products is empty or undefined
  };
